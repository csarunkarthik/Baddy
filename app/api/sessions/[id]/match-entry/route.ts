...
export async function POST(
  ...
  const matchesContext = session.matches.map((m) => {
      ...
      messages: [
        {
          ...
          content:
            ...
            `Fixtures (use these EXACT match numbers):\n${matchesContext}\n\n` +
            "Match the description to the right fixture by looking at the four players (names may be partial or nicknames — pick the closest attendee). Determine the winner team (A or B) using explicit cues like 'beat'/'won'/'lost' AND from the scores if given. Extract scores as integers.\n\n" +
            ...
            "Prefer a fixture that is currently pending (no winner yet) over one already complete. If the description is ambiguous (multiple fixtures fit, or players don't map cleanly), set confidence=\"low\" and explain briefly in note. Always include all keys.",
        },
  ...
  if (parsed.confidence === "low" || !Number.isFinite(parsed.matchNumber)) {
  ...
  const target = session.matches.find((m) => m.matchNumber === parsed.matchNumber);
  if (!target) {
    return NextResponse.json({ error: `No match #${parsed.matchNumber} in this session.` }, { status: 400 });
  }
