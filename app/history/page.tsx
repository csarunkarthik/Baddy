...
export default function HistoryPage() {
  ...
  async function toggleUnlock(sessionId: number, unlock: boolean) {
    if (unlocking.has(sessionId)) return;
    setUnlocking((prev) => new Set(prev).add(sessionId));
    const res = await fetch(`/api/sessions/${sessionId}/unlock`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ forceUnlocked: unlock }),
    });
    if (res.ok) {
      setSessions((prev) =>
        prev.map((s) => (s.id === sessionId ? { ...s, forceUnlocked: unlock } : s))
      );
    }
    setUnlocking((prev) => { const n = new Set(prev); n.delete(sessionId); return n; });
  }
      ...
      <div className="px-4 py-5 max-w-lg mx-auto space-y-3">
        ...
        {loading ? (
          ...
          sessions.map((s) => {
            ...
            const locked = isSessionLocked(s.date, new Date(), s.forceUnlocked);
                  ...
                  <div className="px-5 pb-5 border-t border-gray-50 pt-4 space-y-2">
                    <div className="flex items-center justify-between mb-3 gap-2 flex-wrap">
                      ...
                      <div className="flex items-center gap-2">
                        {locked ? (
                          <button
                            onClick={() => toggleUnlock(s.id, true)}
                            disabled={unlocking.has(s.id)}
                            className="text-xs font-bold text-amber-700 bg-amber-50 hover:bg-amber-100 active:scale-95 disabled:opacity-50 px-3 py-1.5 rounded-full transition-all flex items-center gap-1"
                          >
                            {unlocking.has(s.id) ? (
                              <><span className="w-3 h-3 rounded-full border-2 border-amber-300 border-t-amber-600 animate-spin" />Unlocking…</>
                            ) : <>🔓 Unlock</>}
                          </button>
                        ) : (
                          <>
                            {s.forceUnlocked && (
                              <button
                                onClick={() => toggleUnlock(s.id, false)}
                                disabled={unlocking.has(s.id)}
                                className="text-xs font-bold text-gray-500 bg-gray-100 hover:bg-gray-200 active:scale-95 disabled:opacity-50 px-3 py-1.5 rounded-full transition-all flex items-center gap-1"
                              >
                                {unlocking.has(s.id) ? (
                                  <><span className="w-3 h-3 rounded-full border-2 border-gray-300 border-t-gray-500 animate-spin" />Locking…</>
                                ) : <>🔒 Re-lock</>}
                              </button>
                            )}
                            <button
